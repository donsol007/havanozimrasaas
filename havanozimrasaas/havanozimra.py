...
def get_config_value(doc, fieldname: str) -> str:
    try:
        company_name = doc.company
        print(company_name)
        if not company_name:
            frappe.throw("No company found in the document.")

        # Step 2: Fetch field value from Company Registration using company_name
        doctype = "Company Registration"
        value = frappe.db.get_value(doctype, {"company": company_name}, fieldname)

        frappe.logger().info(f"Fetched {fieldname}='{value}' from {doctype} for company '{company_name}'")
        return value
...
@frappe.whitelist()
def get_token(doc):
    hcloud_baseurl = get_config_value(doc,"custom_server_address")
...
def send_invoice_to_cloud(doc,
    ...
    try:
        ...
        hcloud_baseurl = get_config_value(doc,"custom_server_address")
        hcloud_key = get_config_value(doc,"custom_api_key")
        hcloud_secret = get_config_value(doc,"custom_api_secret")
        devicesn = get_config_value(doc,"custom_device_serial_number")
        if not all([hcloud_baseurl, hcloud_key, hcloud_secret, devicesn]):
            frappe.log_error("Zimra Error",f"One or more user zimra information is missing, Please update Zimra Information")
...
@frappe.whitelist()
def send_from_button(doc, method):
...
def send(doc,method):
    ...
    response_msg =  send_invoice_to_cloud(doc,
